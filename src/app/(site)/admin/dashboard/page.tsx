...
import type { Post, GalleryImage, Testimonial } from "@/lib/types";
import {
  signOut,
  deletePost,
  togglePublish,
...
export default async function Dashboard() {
  ...
  return (
    <section className="max-w-content mx-auto px-6 md:px-10 py-16 md:py-20">
      ...
      <div className="mt-20">
        <div className="flex items-center justify-between mb-8">
          <h2 className="font-display text-2xl">Depoimentos</h2>
          <Link
            href="/admin/dashboard/novo-depoimento"
            className="bg-ink text-cream px-5 py-2.5 text-sm uppercase tracking-widest2 hover:bg-gold hover:text-ink transition-colors focus-ring"
          >
            + Novo depoimento
          </Link>
        ...
        {allTestimonials.length === 0 ? (
          <p className="text-stone">Nenhum depoimento ainda.</p>
        ) : (
          <div className="divide-y divide-ink/10 border-y border-ink/10">
            {allTestimonials.map((t) => (
              <div key={t.id} className="flex items-center gap-5 py-5">
                <div className="relative w-16 h-16 rounded-full overflow-hidden border border-ink/10 shrink-0 bg-paper">
                  <Image
                    src={t.image_url}
                    alt={t.name}
                    fill
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-medium truncate">
                    {t.name} <span className="text-stone font-normal">· {t.city}</span>
                  </p>
                  <p className="text-xs text-stone mt-1 truncate">{t.text}</p>
                </div>
